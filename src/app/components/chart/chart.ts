...
import { blogContent } from '../../core/interfaces/blog-interface';
...
export class MyChart {
  ...
  ngAfterViewInit() {
    ...
    this.browserOnly(() => {
      var currentData = [
        CURRENTCOORDINATES[0].name,
        CURRENTCOORDINATES[0].title,
        CURRENTCOORDINATES[0].comment,
        CURRENTCOORDINATES[0].pictureSettings.src,
      ];

      //let blogContent;
      makeMapChart();
      makeBlogArticle(currentData);
        ...
        secondaryPointSeries.bullets.push(function () {
          ...
          secondaryCircle.events.on('click', function (e) {
            if (e.target.dataItem) {
              ...
              var picture: string = cible.pictureSettings;
              var requestStatus: boolean = cible.requestStatus;
              commentWindow.push(city, title, comment, picture);
            }
            console.log(commentWindow);

            makeBlogArticle(commentWindow);
          });
      ...
      function makeBlogArticle(data: Array<any>) {
        // Create root
        var root = am5.Root.new('blogdiv');

        var chart = root.container.children.push(am5map.MapChart.new(root, {}));

        // Create series
        /*var blogseries = chart.series.push(am5map.MapPointSeries.new(root, {}));
        var test = data[0];
        var datas = [
          { blogCityname: data[0], blogText: data[2], blogTitle: data[1], blogPicture: data[3] },
        ];

        blogseries.data.setAll(datas);

        console.log(blogseries.dataItems);
        ...
        var blogContainer = root.container.children.push(
        ...
        var title = blogContainer.children.push(
          am5.Label.new(root, {
            text: data[1],
        ...
        var comment = blogContainer.children.push(
          am5.Label.new(root, {
            text: data[2],
            fontSize: 23,
            populateText: true,
        ...
        console.log(data[3]);
        let imageContainer = blogContainer.children.push(
        ...
        let imageContainer2 = blogContainer.children.push(
          am5.Container.new(root, {
            html:
              '<div style="text-align: center;"><img src=\"' +
              data[3] +
              '\"width=\"100\" /><p style="font-size: 12px;">{popUpText}</p></div>',
          }),
        );

        //REPRENDRE ICI : trouver comment faire passer les variables blog
        //blogContent = chart.series.push(am5map.MapPointSeries.new(root, {}));

        //var bulletContainer = blogContainer.children.push(am5.Container.new(root, {}));

        /*   blogContent.data.setAll([
          {
            blogCityname: data[0],
            blogText: data[2],
            blogTitle: data[1],
        ...
        var test: any = blogContent.dataItems[0].dataContext;
        console.log(test.blogCityname);
        //   {
        //   latitudeField: 'lat',
        //   longitudeField: 'long',
        //   name: 'city',
        ...
        let label = bulletContainer.children.push(
          am5.Label.new(root, {
            populateText: true,
            html: '<div style="text-align: center;"><p style="font-size: 20px;">{name}</p></div>',
            x: am5.percent(50),
            y: am5.percent(0),
            width: 500,
        ...
        let text = textContainer.children.push(
          am5.Label.new(root, {
            populateText: true,
            html: '<div style="text-align: center;"><p style="font-size: 12px;">{blogTitle}</p></div>',
            x: am5.percent(50),
            y: am5.percent(0),
            fill: am5.color(0xffffff),
            fillOpacity: 1,
        ...
        let imageContainer = textContainer.children.push(
          am5.Container.new(root, {
            html: '<div style="text-align: center;"><img src={blogPicture} width=\"100\" /><p style="font-size: 12px;">{popUpText}</p></div>',
          }),
        );
        ...
        root.container.children.push(bulletContainer);
